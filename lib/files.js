  ...
  firstBuild: (conf) => {
    fs.mkdir(conf.apiName, (e) => {
      if (!e || (e && e.code === "EEXIST")) {
        exec(`cp -R ./template/. ${conf.apiName}/`, (error, stdout, stderr) => {
          if (error) {
            console.log(`error: ${error.message}`);
            return;
          }
          exec(
            `node -e "let pkg=require('./package.json'); pkg.name='${conf.apiName}'; require('fs').writeFileSync('${conf.apiName}/package.json', JSON.stringify(pkg, null, 2));"`,
            (error, stdout, stderr) => {
              if (error) {
                console.log(`error: ${error.message}`);
                return;
